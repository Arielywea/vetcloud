...
function getPrevalenceRank(d: any): number | null {
  const dogRank = d.prevalence_rank_dogs;
  const catRank = d.prevalence_rank_cats;
  if (dogRank && catRank) return Math.round((dogRank + catRank) / 2);
  if (dogRank) return dogRank;
  if (catRank) return catRank;
  return null;
}
...
async function seed() {
  ...
  for (const d of ALL_DISEASES) {
    try {
      ...
      if (ok) { created++; console.log(`  ✓ ${d.name} (rank: ${getPrevalenceRank(d) || 'N/A'})`); }
    } catch (err: any) {
  ...
  console.log('');
  console.log(`════════════════════════════════════════════`);
  console.log(`✓ Done! Created: ${created}/${ALL_DISEASES.length}`);
  console.log('════════════════════════════════════════════');
